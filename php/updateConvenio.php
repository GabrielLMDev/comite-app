...
/*PARAMETROS URL*/
$idContrato = $_GET['n_contrato'];
$idCliente = $_GET['n_cliente'];
...
$Plazo = $_GET['plazo'];
$PlazoActual = $_GET['plazoactual'];
$Restante = $_GET['restanteP'];
$Pago = $_GET['monto'];
...
if ($stmt->execute()) {
    ...
    $stmt->bindParam(':monto', $Pago);
    $stmt->bindParam(':folio', $folio);

    if ($stmt->execute()) {
        SetMov($folio, $Pago, $idCliente, $idEmpleado, $nombreEmpleado, $idPeriodo, $Estatus);
    } else {
...
function setMov($folio, $Pago, $idCliente, $idEmpleado, $nombre, $idPeriodo, $Estatus)
{
    require 'database.php';
    $concept = "PAGO DE CONVENIO POR $" . $Pago . " DE CLIENTE " . $idCliente;
    ...
    if ($stmt->execute()) {
        ...
        echo json_encode('Error 511-620');
    }
}
